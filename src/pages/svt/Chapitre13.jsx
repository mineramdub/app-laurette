...
import { chapitre13 } from '../../data/svtData';
...
export default function Chapitre13() {
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="subject-tag">SVT · Chapitre 13</div>
        <h1>{chapitre13.titre}</h1>
        <p>Sarcomère, filaments d'actine et myosine, plaque motrice, Ca²⁺ et maladies musculaires.</p>
      </div>
      ...
      <div className="exercise-tabs">
        {tabs.map(tab => (
          ...
          >
            {tab.label}
            {isDone(`svt_ch13_${tab.id}`) && tab.id !== 'cours' && (
      ...
      {activeTab === 'cours' && (
        <CourseContent
          cours={chapitre13.cours}
          flashcards={chapitre13.flashcards}
          onMarkRead={() => markDone('svt_ch13_cours')}
          isRead={isDone('svt_ch13_cours')}
        />
      )}
      ...
      {activeTab === 'flashcards' && (
        <div className="card">
          ...
          {isDone('svt_ch13_flashcards') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà parcouru ces flashcards !</div>
          )}
          <Flashcard cards={chapitre13.flashcards} onComplete={() => markDone('svt_ch13_flashcards')} />
        </div>
      )}
      ...
      {activeTab === 'trous' && (
        <div className="card">
          ...
          {isDone('svt_ch13_trous') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà réussi cet exercice !</div>
          )}
          <TexteTrous data={chapitre13.texteTrous} onComplete={() => markDone('svt_ch13_trous')} />
        </div>
      )}
      ...
      {activeTab === 'quiz' && (
        <div className="card">
          ...
          {isDone('svt_ch13_quiz') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà réussi ce quiz !</div>
          )}
          <QuizQuestion questions={chapitre13.quiz} onComplete={() => markDone('svt_ch13_quiz')} />
        </div>
      )}
    </div>
  );
