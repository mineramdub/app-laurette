...
import { chapitre16 } from '../../data/svtData';
...
export default function Chapitre16() {
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="subject-tag">SVT · Chapitre 16</div>
        <h1>{chapitre16.titre}</h1>
        <p>Allostasie, charge allostatique, burnout, maladies cardiovasculaires et gestion du stress.</p>
      </div>
      ...
      <div className="exercise-tabs">
        {tabs.map(tab => (
          ...
          >
            {tab.label}
            {isDone(`svt_ch16_${tab.id}`) && tab.id !== 'cours' && (
      ...
      {activeTab === 'cours' && (
        <CourseContent
          cours={chapitre16.cours}
          flashcards={chapitre16.flashcards}
          onMarkRead={() => markDone('svt_ch16_cours')}
          isRead={isDone('svt_ch16_cours')}
        />
      )}
      ...
      {activeTab === 'flashcards' && (
        <div className="card">
          ...
          {isDone('svt_ch16_flashcards') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà parcouru ces flashcards !</div>
          )}
          <Flashcard cards={chapitre16.flashcards} onComplete={() => markDone('svt_ch16_flashcards')} />
        </div>
      )}
      ...
      {activeTab === 'trous' && (
        <div className="card">
          ...
          {isDone('svt_ch16_trous') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà réussi cet exercice !</div>
          )}
          <TexteTrous data={chapitre16.texteTrous} onComplete={() => markDone('svt_ch16_trous')} />
        </div>
      )}
      ...
      {activeTab === 'quiz' && (
        <div className="card">
          ...
          {isDone('svt_ch16_quiz') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà réussi ce quiz !</div>
          )}
          <QuizQuestion questions={chapitre16.quiz} onComplete={() => markDone('svt_ch16_quiz')} />
        </div>
      )}
    </div>
  );
