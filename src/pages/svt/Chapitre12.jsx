...
import { chapitre12 } from '../../data/svtData';
...
export default function Chapitre12() {
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="subject-tag">SVT · Chapitre 12</div>
        <h1>{chapitre12.titre}</h1>
        <p>Stratigraphie, datation radioactive, fossiles stratigraphiques et reconstitution de l'histoire géologique.</p>
      </div>
      ...
      <div className="exercise-tabs">
        {tabs.map(tab => (
          ...
          >
            {tab.label}
            {isDone(`svt_ch12_${tab.id}`) && tab.id !== 'cours' && (
      ...
      {activeTab === 'cours' && (
        <CourseContent
          cours={chapitre12.cours}
          flashcards={chapitre12.flashcards}
          onMarkRead={() => markDone('svt_ch12_cours')}
          isRead={isDone('svt_ch12_cours')}
        />
      )}
      ...
      {activeTab === 'flashcards' && (
        <div className="card">
          ...
          {isDone('svt_ch12_flashcards') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà parcouru ces flashcards !</div>
          )}
          <Flashcard cards={chapitre12.flashcards} onComplete={() => markDone('svt_ch12_flashcards')} />
        </div>
      )}
      ...
      {activeTab === 'trous' && (
        <div className="card">
          ...
          {isDone('svt_ch12_trous') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà réussi cet exercice !</div>
          )}
          <TexteTrous data={chapitre12.texteTrous} onComplete={() => markDone('svt_ch12_trous')} />
        </div>
      )}
      ...
      {activeTab === 'quiz' && (
        <div className="card">
          ...
          {isDone('svt_ch12_quiz') && (
            <div className="encouragement" style={{ marginBottom: '1.5rem' }}>🌿 Tu as déjà réussi ce quiz !</div>
          )}
          <QuizQuestion questions={chapitre12.quiz} onComplete={() => markDone('svt_ch12_quiz')} />
        </div>
      )}
    </div>
  );
